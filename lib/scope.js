...
function getStatusFromDefinition(nmockDef) {
  //  Backward compatibility for when `status` was encoded as string in `reply`.
  if (!_.isUndefined(nmockDef.reply)) {
    //  Try parsing `reply` property.
    var parsedReply = parseInt(nmockDef.reply, 10);
  ...
  return nmockDef.status || DEFAULT_STATUS_OK;
}

function getScopeFromDefinition(nmockDef) {

  //  Backward compatibility for when `port` was part of definition.
  if (!_.isUndefined(nmockDef.port)) {
    //  Include `port` into scope if it doesn't exist.
    var options = url.parse(nmockDef.scope);
    if (_.isNull(options.port)) {
      return nmockDef.scope + ':' + nmockDef.port;
    } else {
      if (parseInt(options.port) !== parseInt(nmockDef.port)) {
        throw new Error('Mismatched port numbers in scope and port properties of NMock definition.');
      }
    }
  }

  return nmockDef.scope;
}
...
function define(nmockDefs) {
  ...
  nmockDefs.forEach(function(nmockDef) {
    ...
    var response;
    if (!nmockDef.response) {
      response = '';
    } else {
      response = _.isString(nmockDef.response) ? tryJsonParse(nmockDef.response) : nmockDef.response;
    }
    ...
    if (body==="*") {
      ...
      nmock = startScope(nscope, options);
      //  If request headers were specified filter by them.
      if (_.size(reqheaders) > 0) {
        for (var k in reqheaders) {
          nmock.matchHeader(k, reqheaders[k]);
        }
      }
      if (nmockDef.filteringRequestBody) {
        nmock.filteringRequestBody(nmockDef.filteringRequestBody);
      }
      nmock.intercept(npath, method, body).reply(status, response, headers);
    }
